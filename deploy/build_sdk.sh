...
# Build SDKs
rm -rf dist-air
SDK_OUTPUT_CLEAN=1 IS_TWALLETGRAM_WALLET=0 webpack --config webpack-air.config.ts
SDK_OUTPUT_CLEAN=0 IS_TWALLETGRAM_WALLET=1 webpack --config webpack-air.config.ts
...
IOS_TWALLETGRAM_TARGET="mobile/ios/App/App/Resources/TwalletGram/JS"
ANDROID_TWALLET_TARGET="mobile/android/app/src/twallet/assets/js"
ANDROID_TWALLETGRAM_TARGET="mobile/android/app/src/twalletgram/assets/js"

mkdir -p "$IOS_TWALLET_TARGET"
mkdir -p "$IOS_TWALLETGRAM_TARGET"
mkdir -p "$ANDROID_TWALLET_TARGET"
mkdir -p "$ANDROID_TWALLETGRAM_TARGET"
...
rm -f "$IOS_TWALLETGRAM_TARGET"/*-sdk.js "$IOS_TWALLETGRAM_TARGET"/*-sdk.js.LICENSE.txt
cp dist-air/twalletgram-sdk.js "$IOS_TWALLETGRAM_TARGET/"
cp dist-air/twalletgram-sdk.js.LICENSE.txt "$IOS_TWALLETGRAM_TARGET/" 2>/dev/null || true
...
rm -f "$ANDROID_TWALLETGRAM_TARGET"/*-sdk.js "$ANDROID_TWALLETGRAM_TARGET"/*-sdk.js.LICENSE.txt
cp dist-air/twalletgram-sdk.js "$ANDROID_TWALLETGRAM_TARGET/"
cp dist-air/twalletgram-sdk.js.LICENSE.txt "$ANDROID_TWALLETGRAM_TARGET/" 2>/dev/null || true
